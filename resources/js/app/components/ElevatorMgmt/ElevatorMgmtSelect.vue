<template>
    <div :class="[color]">
        <select ref="select2"
                ...
                style="width: 100%;"
                :disabled="disabled">
            <option v-for="row in elevatorMgmts" :value="row.elevator_id">
                {{ row.elevator_id }}
            </option>
        </select>
    </div>
...
export default {
    ...
    async created() {
        if(this.items === null) {
            ...
            this.elevatorMgmts = this.items;
        }
        if(!this.multiple && this.value) {
            this.$emit('update:selected', _.find(this.elevatorMgmts, (r) => {
                return r.elevator_id == this.value;
            }));
        }
        this.setSelect2Val();
    },
    ...
    methods: {
        ...
        async fetchData() {
            ...
            try {
                ...
                if(res.status == 0) {
                    const data = res.data;
                    this.elevatorMgmts = data.elevatorMgmts;
                }
            } catch(err) {
